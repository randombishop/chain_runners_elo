...
class RankingTable extends Component {
  ...
  renderItem(item) {
    return (
      <tr key={item.id} className="ranking-table-row" onClick={this.props.runnerClicked(item)}>
    ...
    if (this.props.data) {
      ...
      return (
        <table style={{ width: '100%' }}>
          ...
          <tbody>{this.renderItems(list)}</tbody>
        </table>
      )
    } else {
      return ''
    ...
    if (this.props.data) {
      return (
        <div style={{ display: 'flex' }}>
          <div>
            <Button color="inherit" disabled={this.state.page === 0} onClick={this.back}>
              <ArrowBack />
            </Button>
          </div>
  ...
  render() {
    return (
      <div className="ranking-panel">
        ...
        <div className="ranking-panel-table">{this.renderList()}</div>
        <div className="ranking-panel-info">
          Last updated:{' '}
          <span className="my-green">
            <strong>{this.props.lastUpdateTimestamp}</strong>
          </span>
          <br />
          (Rating system uses ELO algorithm with the same parameters as MegaCity's Chess Organization)
        </div>
        <div className="ranking-panel-action">{this.renderButtons()}</div>
      </div>
    )
  }
}
